/**
 * CLI version of historical sync that uses process.env instead of SvelteKit's $env
 */
...
export async function syncHistoricalTransactions(
	...
	try {
		while (hasMore) {
			...
			console.log(
				`[HISTORICAL SYNC] Page ${pagesProcessed + 1}: Found ${transactions.length} transactions`
			);
